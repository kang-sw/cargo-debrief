...
- ort CPU revival Phase 3 (`260411-refactor-ort-cpu-revival`): BackendTag enum (Wgpu/OrtCpu) added to IndexData and DepsIndexData headers. INDEX_VERSION 6→7, DEPS_INDEX_VERSION 3→4. Backend mismatch on load returns Ok(None) → forced re-index. Bincode decode errors (old-layout files) also return Ok(None) instead of Err. Phases 4 (NdArray removal) and 5 (smoke test + docs) pending.
